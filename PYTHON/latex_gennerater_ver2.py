...
class LatexEnvironmentGenerator(QMainWindow):
    ...
    def initUI(self):
        ...
        # Tab môi trường
        env_tab = QWidget()
        tab_widget.addTab(env_tab, "Môi trường LaTeX")
        self.setup_environment_tab(env_tab)
        
        # Tab danh sách
        list_tab = QWidget()
        tab_widget.addTab(list_tab, "Chuyển đổi Danh sách")
    ...
    def setup_list_tab(self, tab_widget):
        ...
        self.list_button = QPushButton('Chuyển đổi Danh sách', self)
    ...
    def convert_list(self, conversion_type):
        ...
        try:
            # Lấy text từ input hoặc clipboard
            text = self.get_input_text(self.list_input_text)
            
            if text:
                # Chuyển đổi theo loại được chọn
                result = self.list_converter.convert_list_menu(text, conversion_type)
                
                # Hiển thị kết quả
                self.list_result_text.setText(result)
                
                # Tự động copy kết quả vào clipboard
                ...
                QMessageBox.information(self, 'Thành công', 
                    f'Đã {messages[conversion_type].lower()} và tự động copy vào clipboard')
            else:
                QMessageBox.warning(self, 'Lỗi', 
                    'Không tìm thấy nội dung trong input và clipboard')
    ...
    def get_input_text(self, input_textbox):
            """Lấy text từ input box hoặc clipboard"""
            text = input_textbox.toPlainText().strip()
            
            # Nếu input box trống, lấy từ clipboard
            if not text:
                clipboard = QApplication.clipboard()
                text = clipboard.text().strip()
                
                # Nếu có nội dung trong clipboard, tự động điền vào input box
                if text:
                    input_textbox.setText(text)
                    # Không hiện thông báo để tránh gây phiền nhiễu
